#ifndef VISUALIZE_CRBM_VISIBLE_PROBE_H
#define VISUALIZE_CRBM_VISIBLE_PROBE_H
...
template<typename T>
struct VisualizeCRBMVisibleProbe : public Display < T >
{
    ...
    VisualizeCRBMVisibleProbe    ( ConvolutionalRBM     <T> * p_crbm
                                    ...
                                    )
    {
        ...
        p_crbm_convolution_probe -> get_neuron_inputs  ( p_crbm , selection );
        
        float val;
        T  in_dx = 0.9 / p_crbm_convolution_probe ->  in_input_grid_nx;
        T  in_dy = 0.9 / p_crbm_convolution_probe ->  in_input_grid_ny;
        T out_dx = 1.0 / p_crbm_convolution_probe -> in_output_grid_nx;
        T out_dy = 1.0 / p_crbm_convolution_probe -> in_output_grid_ny;

        glBegin(GL_QUADS);
        for(long oy=0,k=0,o=0;oy<p_crbm_convolution_probe->in_output_grid_ny;oy++)
        {
            for(long ox=0;ox<p_crbm_convolution_probe->in_output_grid_nx;ox++,o++)
            {
                ...
                for(long iy=0;iy<p_crbm_convolution_probe->in_input_grid_ny;iy++)
                {
                    ...
                    {
                        val  = SCALE*p_crbm_convolution_probe ->  input_dat [ k ] ;
                        //val = 1.0f*(val - min_val)/(max_val-min_val);
                        //val += 0.0;

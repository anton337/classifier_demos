#ifndef VISUALIZE_DATA_ARRAY_COLOR_H
#define VISUALIZE_DATA_ARRAY_COLOR_H
...
struct VisualizeDataArrayColor : public Display < T >
{
    ...
    VisualizeDataArrayColor ( long num_elems
                            , long num_vars
                            , long stride
                            , long nx
                            , long ny
                              ...
                              )
    {
        ...
        for(long y=0,k=selection*num_vars;y<ny;y++)
        {
            for(long x=0;x<nx;x++)
            {
                glColor3f(dat[k+x+nx*y],dat[k+x+nx*y+nx*ny],dat[k+x+nx*y+2*nx*ny]);
